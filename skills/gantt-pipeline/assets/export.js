...
let stages=[],stageStyles=new Map(),exportHolidays=new Set();
...
const work=d=>![0,6].includes(new Date(d+'T12:00:00Z').getUTCDay())&&!exportHolidays.has(d);
const styles=[
...
function workbook(project,result){
 ...
 const cfg=result.config||project.settings;stages=project.stages.map(s=>s.label);styles.length=20;stageStyles=new Map();for(const stage of project.stages){stageStyles.set(stage.label,styles.length);styles.push({fill:stage.color.replace('#','').toUpperCase()});}exportHolidays=new Set(cfg.holidays||[]);
 ...
 const calendar=[header(['Дата','День','Рабочий','Примечание']),...days.map(d=>[dateCell(d),['вс','пн','вт','ср','чт','пт','сб'][new Date(d+'T12:00:00Z').getUTCDay()],work(d)?1:0,work(d)?'':exportHolidays.has(d)?'Исключение календаря':'Выходной'])];
 ...
 'xl/worksheets/sheet2.xml':sheet(registry,[10,20,48,9,15,15,22,42,15,15,13,14,14],{freeze:4,filter:true,heights:Object.fromEntries(tasks.map((t,i)=>[i+2,Math.max(32,Math.min(105,Math.ceil(Math.max(t.title.length,(t.reason||'').length,0)/45)*14+8))]))}),
 'xl/worksheets/sheet3.xml':sheet(settings,[46,108],{freeze:1,heights:{3:38,9:40,10:40,11:40,12:40,13:40,14:55}}),
 'xl/worksheets/sheet4.xml':sheet(calendar,[15,12,13,38],{freeze:1,filter:true})};
 return zip(files);
}
